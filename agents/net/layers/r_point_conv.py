...
class PointConv(nn.Module):
    def __init__(self, hidden_size, pos_size: int = 2, aggrs=('min', 'max', 'mean')):
        ...
        self.seqs = nn.ModuleList([
            FFN([(hidden_size + pos_size), hidden_size, hidden_size], layer_norm=False),
            FFN([(hidden_size + pos_size), hidden_size, hidden_size], layer_norm=False),
            FFN([(hidden_size + pos_size), hidden_size, hidden_size], layer_norm=False)
        ])
    ...
    def _get_E(self, x, y, x_pos, y_pos):
        diff = x_pos.unsqueeze(2) - y_pos.unsqueeze(1)
        E = torch.cat([y.unsqueeze(1).repeat(1, x.size(1), 1, 1), diff], dim=-1)
        return E
    ...
    def forward(
        ...
        # for nn, (m, m_pos) in zip(self.nns, [(x_pred, pos_pred), (x_prey, pos_prey), (x_obst, pos_obst)]):
