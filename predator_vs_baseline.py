...
if __name__ == '__main__':
    ...
    predator_config = TD3Config(critic=CriticConfig(),
                                actor=ActorConfig(),
                                entity='predator',
                                # actor_update_freq=1,
                                # soft_update_freq=1,
    ...
    # prey_config = TD3Config(critic=CriticConfig(),
    #                         actor=ActorConfig(),
    #                         entity='prey',
    #                         # actor_update_freq=1,
    #                         # soft_update_freq=1,
    ...
    while True:
        ...
        buffer.append(
            state,
            {'predators': predator_actions.tolist(), 'preys': prey_actions},
            next_state,
            reward,
            done
        )
        ...
        if global_step_count > args.buffer_steps:
            ...
            if (global_step_count) % 5000 == 0:
                predator_agent.save(osp.join(args.ckpt_save_path, f'td3_pred_{global_step_count}.pt'))
                # prey_agent.save(osp.join(args.ckpt_save_path, f'td3_prey_{global_step_count}.pt'))
